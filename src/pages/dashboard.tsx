    ...
    <div className="sticky top-0 h-screen">
      <Sidebar />
        ...
        <div className="flex-1" >
            ...
            <div className='bg-white rounded-2xl border border-black/5 p-6 hover:shadow-lg transition-shadow duration-300'>
              ...
              <div className='flex flex-col gap-1'>
                {announcements.map((item, index) => (
                  <div
                    key={index}
                    className='group flex items-start justify-between gap-3 py-3 px-3 -mx-3 rounded-lg hover:bg-black/[0.02] transition-colors cursor-pointer'
                  >
                    <div className='flex items-start gap-3'>
                      <span className='w-1.5 h-1.5 rounded-full bg-[#B8901F] mt-2 flex-shrink-0' />
                      <div>
                        <p className='text-sm text-black/75 leading-relaxed group-hover:text-black transition-colors'>
                          {item.text}
                        </p>
              ...
              <div className='relative flex flex-col gap-5 pl-4'>
                ...
                {todaysClasses.map((cls, index) => (
                  <div key={index} className='relative flex items-start gap-4'>
                    <span
                      className='absolute -left-4 top-1 w-3 h-3 rounded-full ring-4 ring-white'
          ...
          {/* Grid: exam status + quick actions */}
              ...
              <div className='grid grid-cols-2 sm:grid-cols-3 gap-3'>
                {quickActions.map((action, index) => {
                  ...
                  return (
                    <button
                      key={index}
                      className='group relative flex flex-col items-center justify-center gap-2.5 py-6 px-3 rounded-2xl border border-black/5 hover:border-transparent overflow-hidden transition-all duration-300 text-center'
                    >
