...
const courses = [
  {
    ...
    schedule: "Monday & Wednesday, 8:00 AM – 10:00 AM",
    venue: "Engineering Block A, Room 203",
    prerequisites: "MEE301",
  },
  {
    code: "MEE403",
    title: "Heat Transfer",
    units: 3,
    lecturer: "Prof. A. Bello",
    type: "Core",
    status: "Registered",
    department: "Mechanical Engineering",
    schedule: "Tuesday & Thursday, 10:00 AM – 12:00 PM",
    venue: "Engineering Block A, Room 108",
    prerequisites: "MEE302",
  },
  {
    code: "MEE405",
    title: "Production Engineering",
    units: 2,
    lecturer: "Dr. R. Thomas",
    type: "Core",
    status: "Registered",
    department: "Mechanical Engineering",
    schedule: "Wednesday, 1:00 PM – 3:00 PM",
    venue: "Engineering Block B, Room 12",
    prerequisites: "None",
  },
  {
    code: "MEE407",
  ...
  },
  {
    ...
    units: 3,
    lecturer: "Prof. D. Adeyemi",
    type: "Core",
    status: "Registered",
    department: "Mechanical Engineering",
    schedule: "Monday & Thursday, 2:00 PM – 4:00 PM",
    venue: "Engineering Block A, Room 203",
    prerequisites: "MEE305",
  },
]
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1.5 bg-white border border-black/10 rounded-lg shadow-lg overflow-hidden z-20 w-44">
          <button
            ...
            className="w-full text-left px-4 py-2 text-sm text-black/50 hover:bg-black/[0.03] transition-colors"
          >
            All
          </button>
          {options.map((opt, i) => (
            <button
              ...
              className="w-full text-left px-4 py-2 text-sm text-black/70 hover:bg-black/[0.03] transition-colors"
            >
              {opt}
            </button>
          ))}
  ...
  return (
    <div className="bg-[#F6F6F2] min-h-screen">
      <div className="px-4 md:px-8 py-8 max-w-6xl mx-auto flex flex-col gap-6">
        ...
        <div className="bg-white rounded-2xl border border-black/5 p-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              className="w-full border border-black/10 rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:border-[#1E3A8A] transition-colors"
            />
          </div>
              ...
              <tbody>
                {filteredCourses.map((course, i) => (
                  <tr
                    ...
                    className="border-b border-black/5 last:border-b-0 hover:bg-[#1E3A8A]/[0.03] cursor-pointer transition-colors"
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${
                        course.type === "Core" ? "bg-[#1E3A8A]/10 text-[#1E3A8A]" : "bg-[#B8901F]/10 text-[#B8901F]"
                      }`}>
                        {course.type}
                      </span>
                    </td>
        ...
        {/* Credit summary + Registration status */}
                ...
                <div className="flex flex-col gap-1">
                  {courseActions.map((action, i) => {
                    ...
                    return (
                      <button
                        key={i}
                        className="flex items-center gap-3 text-sm text-black/70 hover:text-[#14263F] hover:bg-[#14263F]/5 px-3 py-2.5 rounded-lg transition-colors -mx-3"
                      >
